...
const DetailUserAnda = () => {
    const navigate = useNavigate();

    // recoil
    const [pelanggan, setPelanggan] = useRecoilState(dataUserAnda)
    const [alert, setAlert] = useRecoilState(alertState)
    const parentId = window.location.href.split('/')[5]
    ...
    const setDataPelanggan = () => {
        setEmpty(false)
        axios.get(`${API}user/customer?search=${search.search}&page=1&status=${search.status}&limit=${search.limit}&parent_id=${parentId}`, {
            headers: {
    ...
    const liveSearch = async (keyword, status) => {
        ...
        try {
            const res = await axios.get(`${API}user/customer?search=${keyword}&page=1&status=${status}&limit=${search.limit}&parent_id=${parentId}`, {
            ...
            buttons: [
              {
                ...
                onClick: () => {
                    ...
                    axios.delete(`${API}user/parent/delete?user_id=${id}`, {
                        headers: {
                            Authorization: 'Bearer ' + localStorage.getItem('authToken')
                        }
                    })
    ...
    return (
        <Box component="div" sx={{
            ...
            <Button onClick={() => navigate(`/admin/user_form/${parentId}`) } size='small' sx={{ my: 2, p:1, maxWidth: 300, borderRadius: 25 }} variant="contained">
               <AddBoxIcon  /> Tambah Pelanggan Anda
            </Button>
                            ...
                            return (
                                <TableRow key={val.id}>
                                    ...
                                    <TableCell align="center">
                                        ...
                                        >
                                            <EditIcon sx={{ color: green[500], cursor: 'pointer' }} onClick={() => navigate(`/admin/user_form/${parentId}-${val.id}`)} />
